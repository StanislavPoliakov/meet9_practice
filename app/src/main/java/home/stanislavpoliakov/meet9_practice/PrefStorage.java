...
public class PrefStorage {
    ...
    /*Поскольку логика программы предполагает, что элементы будут отображаться до того,
    * как будет запущено окно настроек - прописываем дефолтные значения
    *
    * Честно, даже не знаю, стоит ли комментировать этот класс...
    *
    * Изначально все сделал на getStringSet. Однако выяснилось, что сохранение настроек
    * выполнняется в хаотичном порядке, то есть LinkedHashSet сохраняется не по порядку добавления
    * элементов. Переделал на структуру данных*/
